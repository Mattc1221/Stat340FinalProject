...
To do this we will perform some two sample t tests to determine a difference between the average statistics between Astros at home and other teams at their home stadium. To start, let us first test a few more variables specifically for Astros home vs. away.
...
Above we see that the Astros perform much better at home than away in the three variables we chose: Batting Average, Home Runs, and Runs Batted In. This again does not show conclusive evidence that they cheated, but instead adds strength to our earlier analysis that the Astros definitely have a home field advantage. Next we will compare the Astros with the rest of the league, by performing another two sample t test between the astros home games and the home games of all other teams in the league.
...
```{r echo=FALSE}
t.test(a$sOPS.,b$sOPS.)
#t.test(a$tOPS.,b$tOPS.)
t.test(a$sOPS.,c$sOPS.)
