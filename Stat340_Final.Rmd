...
Next we will attempt to perform a regression to predict the percentage of games that a team will win based on their summed statistics in Runs, Hits, Home Runs, and Runs Batted In. To do this we will have to combine a few datasets: the data we have for specific players from baseball-reference.com and data for team statitcs (games won, lost and played by home and away).
...
lm_data_2017_home <- lm_data_2017 %>%
  filter(split == "HOME") %>%
  filter(team != "astros")

lm_data_2017_away <- lm_data_2017 %>%
  filter(split == "AWAY") %>%
  filter(team != "astros")

lm.2017.home <- lm(win_percent ~ R + H + HR + RBI, data=lm_data_2017_home)
lm.2017.away <- lm(win_percent ~ R + H + HR + RBI, data=lm_data_2017_away)
```
```{r}
summary(lm.2017.home)
summary(lm.2017.away)
```

From our results we can see that our regressions are very bad. None of the p values are below .05 for both home and away games. However it is interesting to note that the R-squared value is much higher for Away than for Home games. In the future we will reeally need to improve upon this model.
...
+ We spend a tremendous amount of time reading in new data and cleaning it, in order to form a regression model. After forming models, 
...
+ We want to figure out why the new regression models we created were so bad. Before our the final project is due, we will work on how to improve our model, and attend office hours to gain some insight as why they currently are the way they are. We can then plot the models to vusually see how they are doing 
