...
These box plots show that there seems to be a big difference between the home and away games. 
...
home_2017 <- season_2017 %>%
  filter(split == "HOME") %>%
  select(OPS, R, H, BA, HR, RBI) %>%
  drop_na()

away_2017 <- season_2017 %>%
  filter(split == "AWAY") %>%
  select(OPS, R, H, BA, HR, RBI) %>%
  drop_na()
```
...
+ Plot 2: *Difference of Average Player Runs* - We were not sure which random variable we should use to model the average number of runs a player gets in a season for our monte carlo simulation. We ultimately decided on a poisson random variable due to its discrete nature.
